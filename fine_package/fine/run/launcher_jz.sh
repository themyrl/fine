#!/bin/bash
#SBATCH --job-name=glamv2_word     # job name
...
#SBATCH --output=logs/glamv2_word.out # output file name # add %j to id the job
#SBATCH --error=logs/glamv2_word.err  # error file name # add %j to id the job
#SBATCH -C v100-32g
...
srun python fine_package/fine/run/run.py -network nnUNetTrainerV2_glam -task 140 -outpath GLAMV2 #glamv2_word
...
# srun python fine_package/fine/run/run.py nnUNetTrainerV2_glamUNet_v2 140 notta_GLAMNNUNETV2 1 0 0 #glamv2_word # glam+nnunet with glam at almost all encoder stage
