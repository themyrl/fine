...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, gt_num=1):
        super().__init__()
        self.dim = dim
...
class swintransformer(SegmentationNetwork):
    ...
    def __init__(self, input_channels=1, base_num_features=64, num_classes=14, num_pool=4, num_conv_per_stage=2,
        ...
        self.model_down=SwinTransformer(pretrain_img_size=self.imsize,window_size=window_size,embed_dim=embed_dim,patch_size=patch_size,
                                        depths=depths,num_heads=num_heads,in_chans=input_channels, gt_num=gt_num)
        self.encoder=encoder(pretrain_img_size=self.imsize,embed_dim=embed_dim,window_size=window_size[::-1][1:],patch_size=patch_size,num_heads=[num_heads[2],num_heads[1],num_heads[0]],
                            depths=[2,2,2], gt_num=gt_num)
