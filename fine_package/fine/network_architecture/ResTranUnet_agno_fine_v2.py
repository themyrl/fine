...
from fine.network_architecture.DeTrans.FineDeformableTrans_v2 import FineDeformableTransformer
    ...
    def posi_mask(self, x):
        ...
        for lvl, fea in enumerate(x):
            if lvl > 1:
                ...
                masks.append(torch.zeros((fea.shape[0], fea.shape[2], fea.shape[3], fea.shape[4]), dtype=torch.bool, device=fea.device))#.cuda())
