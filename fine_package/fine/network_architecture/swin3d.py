...
class PatchEmbed(nn.Module): # Ok 3D / ready to debug
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        """Forward function."""
        x = self.patch_embed(x) # ok 3d | ready debug

        Wh, Ww, Wd = x.size(2), x.size(3), x.size(4)
        if self.ape:
            # interpolate the position embedding to the corresponding size
            absolute_pos_embed = F.interpolate(self.absolute_pos_embed, size=(Wh, Ww, Wd), mode='bicubic')
            x = (x + absolute_pos_embed).flatten(2).transpose(1, 2)  # B Wh*Ww*Wd C
        else:
            x = x.flatten(2).transpose(1, 2)
        x = self.pos_drop(x)
        # ok 3d | ready debug
