...
# V3 + We've removed global tokens and only use volume tokens
...
class ClassicAttention(nn.Module):
    ...
    def forward(self, x, pe, mask=None):
        ...
        if pe != None:
            ...
            strt = m//2-N_//2
            pe = pe[strt:strt+N_,:]
            # print("----> fine x shape",x.shape)
            # print("----> fine pe shape",pe.shape)
            # print("----> fine self.n_vts shape",self.n_vts)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, gt_num=8, n_vts=8,vt_num=1, clip=False,
                 vt_map=(3,5,5)):
        super().__init__()
        self.n_vts = n_vts
        self.dim = dim
    ...
    def forward(self, x, mask_matrix, vts_pro, vt_pos=None, check=None):
        """ Forward function.
        ...
        if len(vts.shape) != 3:
            ...
            vt = vts[:, :(x_windows.shape[0]//B)*self.gt_num, :]
            vt = rearrange(vt, "b (w n) c -> (b w) n c", w=x_windows.shape[0]//B)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
                 ...
                 downsample=True,  
                 use_checkpoint=False, gt_num=8, id_layer=0, vt_map=(3,5,5),vt_num=1, clip=False):
        super().__init__()
        self.window_size = window_size
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                gt_num=gt_num,vt_num=vt_num, clip=clip, vt_map=vt_map)
        ...
        # ws_pe = (   (input_resolution[0]//window_size)*gt_num, 
        #             (input_resolution[1]//window_size)*gt_num, 
        #             (input_resolution[2]//window_size)*gt_num    )
    ...
    def forward(self, x, S, H, W, vt_pos, check, vts_pro):
        """ Forward function.
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                ...
                x, vts_pro = blk(x, attn_mask, vts_pro, vt_pos, check)
        if self.downsample is not None:
            ...
            if Ws >= self.window_size:
                ...
                vts_pro = self.downsample(vts_pro, S//self.window_size, H//self.window_size, W//self.window_size)
                vts_pro = rearrange(vts_pro, "(b g) n d -> b n g d", b=x.shape[0])
                vts_pro = rearrange(vts_pro, "b n g d -> (b n) g d")

            return x, S, H, W, x_down, Ws, Wh, Ww, vts_pro
        else:
            return x, S, H, W, x, S, H, W, vts_pro
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self,
                 ...
                 drop_path=0.,
                 norm_layer=nn.LayerNorm,
                 upsample=True, gt_num=8,id_layer=0, vt_map=(3,5,5), vt_num=1, clip=False
                ):
        super().__init__()
        self.window_size = window_size
        self.shift_size = window_size // 2
    ...
    def forward(self, x,skip, S, H, W, vt_pos, check, vts_pro):
        """ Forward function.
        ...
        vts_pro = rearrange(vts_pro, "(b n) g d -> b n g d", b=x.shape[0])
        vts_pro = rearrange(vts_pro, "b n g d -> (b g) n d")
        vts_pro = self.Upsample(vts_pro, S//self.window_size, H//self.window_size, W//self.window_size)
        vts_pro = rearrange(vts_pro, "(b g) n d -> b n g d", b=x.shape[0])
        vts_pro = rearrange(vts_pro, "b n g d -> (b n) g d")
        ...
        for blk in self.blocks:
            # check = (self.vt_check.sum() >= self.vt_map[0]*self.vt_map[1]*self.vt_map[2])
            x_up, vts_pro = blk(x_up, attn_mask, vts_pro, vt_pos, check)
        
        return x_up, S, H, W, vts_pro
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, vt_pos, check):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, S, H, W, x, Ws, Wh, Ww, vts_pro = layer(x, Ws, Wh, Ww, vt_pos, check, vts_pro)
...
class encoder(nn.Module):
    ...
    def forward(self,x,skips,vt_pos, check, vts_pro=None):
        ...
        for i in range(self.num_layers)[::-1]:
            ...
            x, S, H, W, vts_pro = layer(x,skips[i], S, H, W, vt_pos, check, vts_pro)
...
class swintransformer(SegmentationNetwork):
    ...
    def forward(self, x, pos):
        ...
        seg_outputs=[]
        skips, vts_pro = self.model_down(x, vt_pos, self.vt_check >= 1)
        neck=skips[-1]
       
        out=self.encoder(neck,skips,vt_pos, self.vt_check >= 1, vts_pro)
        ...
        # shotmp1 = "tmp_chekc1\n"
        # shovtch = "vt_check\n"
        # for i in range(self.vt_map[0]):
        #     for j in range(self.vt_map[1]):
        #         for k in range(self.vt_map[2]):
        #             shotmp0 += str(int(tmp_check[0, i*self.vt_map[1]*self.vt_map[2] + j*self.vt_map[2] + k, 0].item())) + " "
        #             shotmp1 += str(int(tmp_check[1, i*self.vt_map[1]*self.vt_map[2] + j*self.vt_map[2] + k, 0].item())) + " "
        #             shovtch += str(int(self.vt_check[i*self.vt_map[1]*self.vt_map[2] + j*self.vt_map[2] + k, 0].item())) + " "
