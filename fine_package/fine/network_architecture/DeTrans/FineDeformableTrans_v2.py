...
class FineDeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.all_volume_tokens = torch.nn.Parameter(torch.randn(num_encoder_layers, num_feature_levels, self.vt_map[0]*self.vt_map[1]*self.vt_map[2], d_model*n_gt))
    ...
    def forward(self, srcs, masks, pos_embeds, pos=None):
        ...
        vt_pos = np.sort(np.array(vt_pos).flatten())



        # Get the selected volume tokens from all volume tokens for the encoder
        # vt_pos = np.random.randint(0, 3*5*5, 16)
        tmp = rearrange(self.all_volume_tokens, "m l v d -> m v l d")
        tmp = repeat(tmp, "m v l d -> m b v l d", b=B) # repeat on batch
        tmp = rearrange(tmp, "m b v l d -> m (b v) l d") # align batch/volume tokens
        sel_vt =  tmp[:, vt_pos] #(blocks, batch*n_vt, n_levels, n_gt*d_model)
        sel_vt = rearrange(sel_vt, "m (b n) l d -> m b n l d", b=B) # (blocks, batch, n_vt, n_levels, n_gt*d_model)
        sel_vt = rearrange(sel_vt, "m b n l d -> m b l n d", b=B)   # (blocks, batch, n_levels, n_vt, n_gt*d_model)
        sel_vt = rearrange(sel_vt, "m b l n (g d) -> m b l n g d", g=self.n_gt)   # (blocks, batch, n_levels, n_vt, n_gt, d_model)
        sel_vt = rearrange(sel_vt, "m b l n g d -> m b l (n g) d", g=self.n_gt)   # (blocks, batch, n_levels, n_vt*n_gt, d_model)
        sel_vt = rearrange(sel_vt, "m b l n d -> m b (l n) d", b=B) # (blocks, batch, n_levels*n_vt*n_gt,  d_model)
        ...
        seen_vts = rearrange(seen_vts, "m (b n) p c -> m b n p c", b=B) # (blocks, batch, n_seen, n_levels, n_gt*d_model)
        seen_vts = rearrange(seen_vts, "m b n p (g c) -> m b n p g c", g=self.n_gt) # (blocks, batch, n_seen, n_levels, n_gt, d_model)
        seen_vts = rearrange(seen_vts, "m b n p g c -> m b p (n g) c") # (blocks, batch, n_levels, n_seen*n_gt, d_model)
...
class FineDeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, seen_vts=None, sel_vt=None):
        ...
        sel_vt_used = sel_vt[0, ...]
        for blk, layer in enumerate(self.layers):
            if blk != 0:
                sel_vt_used += sel_vt[blk, ...]
            output, _, sel_vt_used = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask, seen_vts=seen_vts[blk, ...], sel_vt=sel_vt_used)
