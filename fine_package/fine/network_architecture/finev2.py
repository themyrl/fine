...
# We've changed old version of volume token selction to be in 3D
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, gt_num=1, n_vts=8,vt_num=1, clip=False):
        super().__init__()
        self.n_vts = n_vts
        self.dim = dim
    ...
    def forward(self, x, mask_matrix, gt, pe, vts, vt_pos, check):
        ...
        # print("--> check_pos" ,check.shape)
        # print("--> check.nonzero(as_tuple=True)[0]", check.nonzero(as_tuple=True)[0].shape)
        check_pos = rearrange(check.nonzero(as_tuple=True)[0], '(b n) -> b n', b=B)
        vt_mask = torch.zeros((B, vts.shape[1]*self.vt_num, vts.shape[1]*self.vt_num), dtype=vts.dtype, device=vts.device)-1000
        # print("--> check_pos" ,check_pos.shape)
        # print("--> vt_mask" ,vt_mask.shape)
        # print("--> vts" ,vts.shape)
        vt_mask[check_pos] = 0
...
class swintransformer(SegmentationNetwork):
    ...
    def __init__(self, input_channels=1, num_classes=14, deep_supervision=True,
                gt_num=1, vt_map=(3,5,5), imsize=[64,128,128], vt_num=1, 
    ...
    def forward(self, x, pos):
        B = x.shape[0]
        vt_pos = []
        tmp_check = torch.zeros(B, self.vt_map[0]*self.vt_map[1]*self.vt_map[2],1)
        ...
        seg_outputs=[]
        skips = self.model_down(x, vt_pos, tmp_check >= 1)
        neck=skips[-1]
       
        out=self.encoder(neck,skips,vt_pos, tmp_check >= 1)
