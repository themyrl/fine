#!/bin/bash
#SBATCH --job-name=nnfo_word_eval     # job name
...
#SBATCH --output=logs/nnfo_word_eval.out # output file name # add %j to id the job
#SBATCH --error=logs/nnfo_word_eval.err  # error file name # add %j to id the job
# # SBATCH -C v100-32g
...
srun python fine_package/fine/run/run.py nnUNetTrainerV2_nnFormer 140 NNFORMER 0 1 #nnfo_word_eval
# srun python fine_package/fine/run/run.py nnUNetTrainerV2_fine 140 FINE 0 1 #fine_word_eval
