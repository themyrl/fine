#!/bin/bash
#SBATCH --job-name=finv32_livus_nta_c     # job name
#SBATCH --ntasks=1                  # number of MP tasks
#SBATCH --ntasks-per-node=1          # number of MPI tasks per node
#SBATCH --gres=gpu:1                 # number of GPUs per node
#SBATCH --cpus-per-task=20   #10           # number of cores per tasks
#SBATCH --hint=nomultithread         # we get physical cores not logical
#SBATCH --time=50:10:00             # maximum execution time (HH:MM:SS)
#SBATCH --qos=qos_gpu-t4
#SBATCH --output=logs/finv32_livus_nta_c.out # output file name # add %j to id the job
#SBATCH --error=logs/finv32_livus_nta_c.err  # error file name # add %j to id the job
#SBATCH -C v100-32g
...
# srun python fine_package/fine/run/run.py -network nnUNetTrainerV2_finev3UNet_v2 -task 017 -outpath notta_FINEV32NNUNETV2 -na #funv32_bcv_nta
